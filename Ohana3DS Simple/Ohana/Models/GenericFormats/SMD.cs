        ...
        public static void export(RenderBase.OModelGroup model, string fileName, int modelIndex, int skeletalAnimationIndex = -1)
        {
            ...
            else
            {
                ...
                if (error) Console.WriteLine(
                    "Warning: One or more bones uses an animation type unsupported by Source Model!");
            }
            output.AppendLine("end");
            ...
            while (reader.BaseStream.Position < reader.BaseStream.Length)
            {
                ...
                switch (parameters[0])
                {
                    case "version":
                        ...
                        {
                            Console.WriteLine(
                                "SMD Importer: Corrupted SMD file! The version isn't specified!");
                            return null;
                        }
                        else if (parameters[1] != "1")
                        {
                            Console.WriteLine("SMD Importer: Unknown SMD version!");
                            return null;
                        }
                        break;
                    case "nodes":
